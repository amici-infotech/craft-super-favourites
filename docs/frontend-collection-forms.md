...
The `_self.fieldErrors()` macro renders validation errors from the model Craft returns after a failed submit. Use the returned `collection` variable when it exists so submitted values and errors are preserved. AJAX save/delete responses include a boolean `success` value. Collection saves validate the whole collection element at once, including global/default permissions. Collection delete failures also return the failed `collection` model, so list pages can read `collection.getErrors()`.
