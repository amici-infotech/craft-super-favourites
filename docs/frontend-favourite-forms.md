...
Favourite save, add, remove, toggle, move, and delete responses use the `favourite` model name. Failed normal form submissions return the failed `favourite` model with errors attached. AJAX success and failure responses include a boolean `success` value and a `favourite` payload; failures also include the `errors` payload from Craft's model response. Save/add/toggle actions validate the whole favourite item element at once, so missing collections, invalid collections, ownership errors, disallowed element types, duplicate favourites, and custom field errors can be displayed together.
